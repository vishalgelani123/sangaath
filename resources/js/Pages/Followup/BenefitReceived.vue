            ...
            <div class="flex items-center justify-between">
				<div class="w-full border-b border-gray-200">
					<nav class="-mb-px flex" aria-label="Tabs">
						<Link href="/followup/today" :class="['border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm']" aria-current="page">Today</Link>
						<Link href="/followup/previous" :class="['border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm']" aria-current="page">Previous Pending</Link>
						<Link :class="['border-blue-500 text-blue-600', 'w-1/3 py-4 px-1 text-center border-b-2 font-semibold text-sm']" aria-current="page">Benefit Received</Link>
						<Link href="/followup/rejected" :class="['border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm']" aria-current="page">Rejected</Link>
					</nav>
				</div>
            ...
            <div v-if="initial_forms.length > 0" class="mt-6 px-4 sm:px-6 lg:px-8">
                <div class="max-w-sm">
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <TextInput v-model="search_query" v-on:keyup="debouncedSearch" v-on:change="debouncedSearch" autocomplete="off" id="search" type="text" placeholder="Search Benefit Received"
                                ...
                                <DialogPanel
                                    ...
                                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pt-5 sm:pb-4">
                                        <div class="mb-5 flex justify-between items-center">
											<div>
												<DialogTitle as="h3"
													class="text-lg font-medium leading-6 text-gray-900">{{ application_form.popup_title }}</DialogTitle>
												<div class="mt-1">
													<p class="text-sm text-gray-500">Select the details to update status</p>
												</div>
											</div>
										</div>
										<div class="my-5 flex flex-col gap-y-5">
											<div class="flex justify-between">
												...
												<Switch v-model="application_form.has_benefitted" :class="[application_form.has_benefitted ? 'bg-blue-600' : 'bg-gray-200', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2']">
													<span class="sr-only">Use setting</span>
													<span :class="[application_form.has_benefitted ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none relative inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out']">
													<span :class="[application_form.has_benefitted ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']" aria-hidden="true">
														<svg class="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 12 12">
														<path d="M4 8l2-2m0 0l2-2M6 6L4 4m2 2l2 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
														</svg>
													</span>
													<span :class="[application_form.has_benefitted ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']" aria-hidden="true">
											...
											<div class="">
												<InputLabel for="application_date" :value="application_form.has_benefitted ? 'Date' : 'Follow Up Date'" />

												<div class="relative rounded-md shadow-sm">
													<TextInput v-model="application_form.date.value"
														id="application_date" type="date" class="block w-full font-medium" required />
												</div>
												<p v-if="application_form.date.error" class="mt-1 text-red-600 text-sm">Please select valid date</p>
											</div>
											<div v-if="!application_form.has_benefitted" class="">
												...
												<p v-if="application_form.reject_reason.error" class="mt-1 text-red-600 text-sm">Please enter reject reason</p>
											</div>
											<div v-if="!application_form.has_benefitted" class="">
												...
												<div class="relative rounded-md shadow-sm">
													<TextInput v-model="application_form.discrepancy.value"
														id="discrepancy" type="text" class="block w-full font-medium" required />
												</div>
												<p v-if="application_form.discrepancy.error" class="mt-1 text-red-600 text-sm">Please enter discrepancy</p>
											</div>
										</div>
                                    </div>
                                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
										...
										<button v-if="!application_form.processing" @click="application_form.show_popup = false" type="button"
    ...
    export default {
        ...
        methods: {
			...
			loadMoreData(is_first_search = false) {
				...
				axios.get(`/followup/benefit-received?is_ajax=1&skip=${skipping_rows}${search_query ?? ''}`).then(function (response) {
